...
export function generateMockOfferText(caseView: CaseView, offer: Offer): AiOfferText {
  ...
  return {
    customerText:
      ...
      `und des indikativen Marktwerts von ${eur(offer.marketValue)} ergibt sich für das Modell ${offer.model} ` +
      `eine indikative Auszahlung von ${eur(offer.payoutAmount)}. Das Wohnrecht wurde mit ${years} angesetzt. ` +
      "Der nächste Schritt ist die interne Finalprüfung und ein persönliches Beratungsgespräch.",
    partnerSummary:
      `ENTWURF: Angebot für ${customerName}: Marktwert ${eur(offer.marketValue)}, indikative Auszahlung ${eur(offer.payoutAmount)}, ` +
