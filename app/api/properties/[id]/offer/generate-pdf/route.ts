...
import { buildIndicativeOfferPdfData, createIndicativeOfferDocx, getIndicativeOfferTemplateFileName, getIndicativeOfferTemplateKind, sanitizePdfFileName } from "@/lib/pdf-generator";
...
export async function POST(request: Request, { params }: { params: { id: string } }): Promise<Response> {
  ...
  try {
    ...
    if (!caseView) throw new Error("Für die PDF-Erstellung fehlen Objektdaten.");
    if (!canCalculateOffer(user, caseView.property)) throw new Error("Forbidden");
    if (!caseView.customer?.id) throw new Error("Für die PDF-Erstellung fehlen Kundendaten.");

    const body = await request.json().catch(() => ({}));
    const requestedModel = typeof body?.model === "string" ? body.model : undefined;
    const offer = caseView.offers
      .filter((item) => item.kind === "indicative" && (!requestedModel || item.model === requestedModel))
      .sort((a, b) => new Date(b.updatedAt || b.createdAt).getTime() - new Date(a.updatedAt || a.createdAt).getTime())[0];
    if (!offer) throw new Error("Bitte berechnen Sie zuerst das unverbindliche Angebot.");
    ...
    const pdfData = buildIndicativeOfferPdfData(caseView, offer, advisor ?? { name: user.name, email: user.email });
    const template = await readFile(join(process.cwd(), "templates", getIndicativeOfferTemplateFileName(offer.model)));
    const docxBuffer = createIndicativeOfferDocx(template, pdfData, getIndicativeOfferTemplateKind(offer.model));
    const fileName = sanitizePdfFileName(`Indikatives_Angebot_${pdfData.caseNumber}_${caseView.customer.lastName || "Kunde"}.pdf`);
    ...
    const document = await prisma.$transaction(async (tx) => {
      const created = await tx.document.create({
        data: {
          ...
          fileName,
          displayName: "Indikatives Angebot",
          ...
          status: "ok",
          scanStatus: "clean",
          scanNote: "Systemseitig aus dem unverbindlichen Angebot erzeugt."
        }
      });
    ...
    await addDbActivity(params.id, user.id, "indicative_offer_pdf_created", "Indikatives Angebot als PDF erstellt.", {
      source: "admin",
      entityType: "document",
